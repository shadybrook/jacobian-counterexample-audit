...
23. The slice theorem is an instance of a general residue-ratio obstruction:
    two rational observables with equal simple poles and nonconstant residue
    ratio admit no nonconstant polynomial combination regular along the
    divisor.
...
26. At a smooth discriminant point, contact order `k` gives escape pole order
    ...
    cubic `W^3+A*W+B`, the nondegenerate pole order is
    `min(alpha,2*beta/3)`.
27. In the all-degree family, generic degrees `n >= 5` are not solvable by
    radicals. At good finite-field reductions, rational-fiber counts are
    asymptotically governed by the fixed-point distribution of `S_n`.
